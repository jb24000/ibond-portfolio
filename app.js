import {estimateBond,fmtMonthKey,currentIssueInfo} from './ibond.js';
...
async function syncDrive(){
 try{
  ...
  if(!file)await uploadDrive(null,local);
   ...
   const cloud=await downloadDrive(file.id);if(!Array.isArray(cloud.bonds)||!cloud.bonds.every(validBond))throw Error('Cloud portfolio contains invalid data');
